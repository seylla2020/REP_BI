...
SELECT
			...
			M.CUS_CUST_ID,
			CASE WHEN CCC.CUS_FIRST_NAME IS NULL THEN COALESCE(CCC.CUS_FIRST_NAME,  CCC.CUS_LAST_NAME) 
					 ELSE CCC.CUS_FIRST_NAME || ' ' || CCC.CUS_LAST_NAME 
			END	AS CUS_FULL_NAME,	
			CCC.CUS_CUST_DOC_NUMBER 															  AS CUS_CUST_DOC_NUMBER,
			CCC.CUS_CUST_DOC_TYPE																		AS CUS_CUST_DOC_TYPE,
			CCC.CUS_ADDRESS																					AS CUS_ADDRESS,
			CCC.CUS_RU_SINCE_DT																			AS CUS_RU_SINCE_DT,
			CCC.CUS_RU_SINCE_DT_MP                                  AS CUS_RU_SINCE_DT_MP,
			CCC.CUS_CUST_STATUS_MP                                  AS CUS_CUST_STATUS_MP,
			COALESCE(CCC.CUS_PHONE1, CCC.CUS_CELULAR) 							AS CUS_PHONE,
			CCC.CUS_COUNTRY 																				AS CUS_COUNTRY,
			CCC.CUS_STATE                                           AS CUS_STATE,
			CCC.CUS_CITY																						AS CUS_CITY,
			CCC.CUS_ZIP_CODE																				AS CUS_ZIP_CODE,
			M.CUS_CUST_ID_BUY,
...
LEFT JOIN WHOWNER.LK_CUS_CUSTOMERS_DATA CCC ON CCC.CUS_CUST_ID = M.CUS_CUST_ID AND CCC.SIT_SITE_ID_CUS = 'MLB'
